...
RESPONSES_JSON = env_str("RESPONSES_JSON", "comment_replies.json")
...
# =========================
# BLUE SEND BUTTON FIX
# =========================

def click_blue_send_button(page, editor, reply_text: str) -> bool:
    """
    Facebook's Enter key can select mention suggestions instead of sending.
    This function clicks the blue send arrow/button on the right side of the reply composer.
    """
    ...
    def editor_still_contains_reply() -> bool:
        try:
            ...
            # If the editor disappeared, it was probably sent.
            return False

    try:
        page.wait_for_timeout(500)

        # Find the big grey reply composer around the contenteditable editor.
        composer_box = editor.evaluate(
            """
            (editor) => {
                function getVisibleBox(el) {
                    if (!el) return null;
                ...
                for (let depth = 0; node && depth < 20; depth++) {
                    const box = getVisibleBox(node);

                    if (box) {
                        /*
                          Facebook reply composer is normally:
                          - wide
                          - grey
                          - around 40px to 180px high
                        */
                        if (
                            box.width >= 250 &&
                            box.height >= 35 &&
        ...
        if not composer_box:
            print("[!] Could not find composer box around reply editor.")
            return False

        print(f"[+] Composer box found: {composer_box}")

        # Try to find the rightmost clickable icon/button inside the composer.
        button_box = editor.evaluate(
            """
            (editor) => {
                ...
                const rawCandidates = Array.from(
                    composer.querySelectorAll(
                        '[role="button"], [aria-label], div[tabindex="0"], span[tabindex="0"], svg'
                    )
                );
                ...
                for (const raw of rawCandidates) {
                    const clickable =
                        raw.closest('[role="button"], [aria-label], div[tabindex="0"], span[tabindex="0"]') ||
                        raw;
                ...
                for (const el of unique) {
                    ...
                    const insideComposer =
                        box.cx >= composerRect.left &&
                        box.cx <= composerRect.right &&
                        box.cy >= composerRect.top &&
                        box.cy <= composerRect.bottom;
                ...
                /*
                  Send arrow is usually:
                  - the rightmost clickable thing
                  - near the bottom/right of the composer
                */
                candidates.sort((a, b) => {
                    if (b.right !== a.right) {
        ...
        # 1) Detected clickable button/icon.
        if button_box:
            click_points.append(
                (
                    button_box["x"] + button_box["width"] / 2,
                    button_box["y"] + button_box["height"] / 2,
                    "detected rightmost button/icon",
                )
            )

        # 2) Main fallback: bottom-right of grey composer.
        click_points.append(
            (
                composer_box["right"] - 30,
                composer_box["bottom"] - 30,
                "composer bottom-right",
            )
        )

        # 3) Right-middle fallback.
        click_points.append(
            (
                composer_box["right"] - 30,
                composer_box["y"] + composer_box["height"] / 2,
                "composer right-middle",
            )
        )

        # 4) Very far right fallback.
        click_points.append(
            (
                composer_box["right"] - 18,
                composer_box["bottom"] - 28,
                "composer far-right bottom",
            )
        )

        for x, y, label in click_points:
            print(f"[+] Trying send click: {label} at x={x:.1f}, y={y:.1f}")

            page.mouse.click(x, y)
            page.wait_for_timeout(1500)

            if not editor_still_contains_reply():
                print("[+] Reply appears sent.")
                return True

        print("[!] Send click attempts finished, but reply text is still inside editor.")
        return False

    except Exception as e:
        print(f"[!] click_blue_send_button failed: {e}")
        return False
...
def submit_reply(page, article, author: str, reply_text: str) -> bool:
    ...
    try:
        ...
        # If Facebook inserted the person's name, move to the end and type after it.
        # Do NOT press Enter.
        if existing_text:
        ...
        sent = click_blue_send_button(page, editor, reply_text)

        if not sent:
            print(f"[!] Could not click blue send button for {author}")
            return False
